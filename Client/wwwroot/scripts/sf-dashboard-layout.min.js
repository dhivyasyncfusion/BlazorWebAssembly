(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["sf-dashboard-layout"],{
...
window.sfBlazor.DashboardLayout = function () {
  ...
  var SfDashboardLayout = /** @class */function () {
    function SfDashboardLayout(dataId, element, dotnetRef) {
      ...
      // Defines whether the drag operation is performed or not.
      this.isDragged = false;
      // Defines whether the refresh method is called or not.
    ...
    SfDashboardLayout.prototype.initialize = function (property) {
      ...
      this.bindEvents();
      // Defines the offset height and width of the parent element.
      var parentOffset = this.calculateCellSize();
      // Defines the id and header height of the panel.
      var panelHeaderModel = this.contentHeight();
      return JSON.stringify({
        parentOffset: parentOffset,
        enableMediaQuery: this.isMediaQuery,
        panelHeaderModel: panelHeaderModel
      });
    };
    ...
    SfDashboardLayout.prototype.movePanel = function (id, row, col) {
      ...
      this.startRow = this.dragRow = datamodel.row;
      this.startCol = this.dragColumn = datamodel.col;
    ...
    SfDashboardLayout.prototype.removePanel = function (id) {
      ...
      for (var i = this.panelElements.length - 1; i >= 0; i--) {
    ...
    SfDashboardLayout.prototype.onPropertyChange = function (property) {
      ...
      if (property.CellSpacing !== this.cellSpacing) {
        this.cellSpacing = property.CellSpacing;
        this.calculateCellSizeValue();
      }
    ...
    SfDashboardLayout.prototype.swapItems = function (collisions, element, panelModel) {
      ...
      this.startRow = this.dragRow = panelModel.row;
      this.updateOldRowColumn();
    ...
    SfDashboardLayout.prototype.sortCollisionItems = function (collisionItems) {
      ...
      var _loop_1 = function _loop_1(row) {
        ...
        var _loop_2 = function _loop_2(column) {
    ...
    SfDashboardLayout.prototype.setRootHeight = function (height) {
    ...
    SfDashboardLayout.prototype.updatePanelPosition = function (id, col, row) {
      ...
      if (!this.isResizing && !this.isDragged && !this.isRefreshCalled) {
        this.updatePanelProps(id);
      }
    };
    ...
    // updates the panels props after position changes occur during dragging/resizing
    SfDashboardLayout.prototype.updatePanelProps = function (id) {
      if (this.isRenderComplete) {
        var clonedPanels = this.checkForChanges();
        this.panelsInitialModel = this.cloneModels(this.panels);
        if (clonedPanels.length > 0) {
          for (var i = 0; i < clonedPanels.length; i++) {
            if (this.changedId && this.changedId.indexOf(clonedPanels[i].id) === -1) {
              this.changedId.push(clonedPanels[i].id);
            }
          }
          // Calls the server method to update the panel properties of the component based on position changes
          this.dotnetRef.invokeMethodAsync('UpdatedPanelsValues', clonedPanels);
        }
      }
    };
        ...
        var cellElement = collections[i];
        {
          this.dragobj = new sf.base.Draggable(cellElement, {
            ...
            dragStop: function dragStop(args) {
              ...
              _this.updatePanelProps(model.id);
    ...
    SfDashboardLayout.prototype.onDragStart = function (args) {
      ...
      if (this.allowPushing) {
        ...
        if (this.dragColumn !== endCol || this.dragRow !== endRow) {
          ...
          if (this.allowPushing) {
            ...
            this.moveItemsUpwards();
            this.updatePanelProps(panelModel.id);
          }
        }
      }
      ...
      if (this.startCol !== dragCol) {
        this.startCol = this.dragColumn = endCol;
        this.moveItemsUpwards();
      }
      if (!this.allowPushing) {
        this.setHolderPosition(args);
    ...
    SfDashboardLayout.prototype.bindEvents = function () {
      this.refreshListener = this.refresh.bind(this, true);
      // eslint-disable-next-line
     ...
     * @public
     */
    SfDashboardLayout.prototype.refresh = function (isResize) {
      if (this.dotnetRef && document.getElementById(this.element.id)) {
        this.panelsSizeY = 0;
        this.updateDragArea();
          ...
          if (this.element.classList.contains(RESPONSIVE)) {
            ...
            for (var i = 0; i < this.element.querySelectorAll('.e-panel').length; i++) {
              ...
              if (this.panels.length != 0) {
                ...
                if (!sf.base.isNullOrUndefined(cellInstance)) {
        ...
        this.dotnetRef.invokeMethodAsync('CalculateSize', this.calculateCellSize(), this.isMediaQuery, isResize, this.contentHeight());
        this.resizeEvents();
        this.checkDragging(this.dragCollection);
        this.isRefreshCalled = false;
      }
    ...
    SfDashboardLayout.prototype.resizeEvents = function () {
      if (this.allowResizing) {
        ...
        for (var i = 0; i < panelElements.length; i++) {
          var eventName = sf.base.Browser.info.name === 'msie' ? 'mousedown pointerdown' : 'mousedown';
          sf.base.EventHandler.remove(panelElements[i], eventName, this.downResizeHandler);
          sf.base.EventHandler.add(panelElements[i], eventName, this.downResizeHandler, this);
          if (sf.base.Browser.info.name !== 'msie') {
            sf.base.EventHandler.remove(panelElements[i], 'touchstart', this.touchDownResizeHandler);
    ...
    SfDashboardLayout.prototype.upResizeHandler = function (event) {
      ...
      if (resizeElement) {
        ...
        var height = panelModel.sizeY * this.getCellSize()[1] + (panelModel.sizeY - 1) * this.cellSpacing[1];
        var width = panelModel.sizeX * this.getCellSize()[0] + (panelModel.sizeX - 1) * this.cellSpacing[0];
        var left = panelModel.col * (this.getCellSize()[0] + this.cellSpacing[0]);
        var top_1 = panelModel.row * (this.getCellSize()[1] + this.cellSpacing[1]);
        sf.base.setStyleAttribute(resizeElement, {
          'left': left + 'px',
          'top': top_1 + 'px',
          'height': height + 'px',
          'width': width + 'px'
    ...
    SfDashboardLayout.prototype.resizingPanel = function (targetElement, panelModel, currentX, currentY) {
      ...
      targetElement.style.top = this.elementY + 'px';
      targetElement.style.left = this.elementX + 'px';
      targetElement.style.width = this.elementWidth + 'px';
      targetElement.style.height = this.elementHeight + 'px';
    ...
    SfDashboardLayout.prototype.pixelsToColumns = function (pixels, isCeil) {
      if (isCeil) {
        return Math.round(pixels / this.cellSize[0]);
      } else {
        return Math.round(pixels / (this.cellSize[0] + this.cellSpacing[0]));
      }
    };
    ...
    SfDashboardLayout.prototype.getResizeRowColumn = function (item) {
      ...
      if (['e-north', 'e-south'].indexOf(this.handleClass) === -1) {
        if (this.handleClass === 'e-west' || this.handleClass === 'e-south-west' ? isColChanged : true) {
    ...
    SfDashboardLayout.prototype.downResizeHandler = function (event) {
      var el = sf.base.closest(event.currentTarget, '.e-panel');
    ...
    SfDashboardLayout.prototype.downHandler = function (event) {
      ...
      this.previousCol = this.getCellInstance(targetElement.id).col;
    };
    SfDashboardLayout.prototype.onDraggingStart = function (args) {
      ...
      this.startCol = this.dragColumn = datamodel.col;
      var eleRowValue = this.startRow = this.dragRow = parseInt(args.element.getAttribute('data-row'), 10);
      var eleSizeY = parseInt(args.element.getAttribute('data-sizeY'), 10);
      this.dotnetRef.invokeMethodAsync('TriggerDragStart', args.element.id, eleRowValue, eleSizeY, eleSizeY);
      this.rows = eleRowValue + eleSizeY >= this.rows ? this.rows + eleSizeY : this.rows;
      this.updateDragArea();
      this.shadowEle = document.createElement('div');
      this.shadowEle.classList.add('e-holder');
    ...
    SfDashboardLayout.prototype.checkForChanges = function () {
      ...
      for (var i = 0; i < this.panels.length; i++) {
        if (this.panels[i].row !== this.panelsInitialModel[i].row || this.panels[i].col !== this.panelsInitialModel[i].col || this.panels[i].sizeX !== this.panelsInitialModel[i].sizeX || this.panels[i].sizeY !== this.panelsInitialModel[i].sizeY) {
          ...
          this.panels[i].sizeY = this.panels[i].sizeY < this.panels[i].minSizeY ? this.panels[i].minSizeY : this.panels[i].sizeY;
        }
      }
  ...
  // tslint:disable-next-line
  var DashboardLayout = {
    initialize: function initialize(dataId, element, dotnetRef, property) {
      new SfDashboardLayout(dataId, element, dotnetRef);
      var instance = window.sfBlazor.getCompInstance(dataId);
      return !sf.base.isNullOrUndefined(instance) ? instance.initialize(property) : null;
    },
    movePanel: function movePanel(dataId, id, row, col) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (!sf.base.isNullOrUndefined(instance)) {
        instance.movePanel(id, row, col);
      }
    },
    removePanel: function removePanel(dataId, id) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (!sf.base.isNullOrUndefined(instance)) {
        instance.removePanel(id);
      }
    },
    resizePanel: function resizePanel(dataId, id, sizeX, sizeY) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (!sf.base.isNullOrUndefined(instance)) {
        instance.resizePanel(id, sizeX, sizeY);
      }
    },
    removeAll: function removeAll(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (!sf.base.isNullOrUndefined(instance)) {
        instance.removeAll();
      }
    },
    addPanel: function addPanel(dataId, panel, rowValue) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (!sf.base.isNullOrUndefined(instance)) {
        instance.rows = rowValue;
        instance.addPanel(panel);
      }
    },
    onPropertyChange: function onPropertyChange(dataId, property) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (!sf.base.isNullOrUndefined(instance)) {
        instance.onPropertyChange(property);
      }
    },
    ...
    updatePanelModels: function updatePanelModels(dataId, props) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (!sf.base.isNullOrUndefined(instance)) {
        instance.clearPanelElements();
        instance.getProperty(props);
        instance.initialize(props);
      }
    },
    setRootHeight: function setRootHeight(dataId, height) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (!sf.base.isNullOrUndefined(instance)) {
        instance.setRootHeight(height);
      }
    }
  };
...
}]);(async()=>{await import(`${document.baseURI}_content/Syncfusion.Blazor/scripts/syncfusion-blazor-base.min.js?v=19.3.1`).then(()=>{sfBlazor.loadDependencies('sfdashboardlayout');})})();