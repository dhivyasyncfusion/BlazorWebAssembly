(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["sf-smith-chart"],{
...
window.sfBlazor.SmithChart = function () {
  ...
  var SfSmithChart = /** @class */function () {
    function SfSmithChart(dataId, id, element, dotNetRef) {
      window.sfBlazor = window.sfBlazor;
      this.mouseY = 0;
      this.mouseX = 0;
      this.resizeTo = 0;
    ...
    SfSmithChart.prototype.wireEvents = function () {
      ...
      sf.base.EventHandler.add(this.element, 'click', this.chartOnMouseClick.bind(this), this);
      sf.base.EventHandler.add(this.element, cancelEvent, this.mouseLeave.bind(this), this);
    ...
    SfSmithChart.prototype.mouseMove = function (e) {
      ...
      if (sf.base.Browser.isDevice || e.type.indexOf('touch') > -1) {
        setTimeout(function (dotNetRef) {
          dotNetRef.invokeMethodAsync('OnSmithChartMouseLeave');
        }, 1000, this.dotNetRef);
      }
    };
  ...
  var SmithChart = {
    ...
    mouseY: 0,
    mouseX: 0,
    initialize: function initialize(dataId, id, dotNetRef, height, width, element) {
      if (element) {
        var navigator_1 = new SfSmithChart(dataId, id, element, dotNetRef);
    ...
    doLinearAnimation: function doLinearAnimation(animationOptions) {
      var _this = this;
      animationOptions.forEach(function (option) {
        var clipRect = document.getElementById(option[0]);
        if (clipRect) {
    ...
    fadeOut: function fadeOut(dataId) {
      ...
      if (sf.base.isNullOrUndefined(chartInstance) || !sf.base.isNullOrUndefined(chartInstance) && sf.base.isNullOrUndefined(chartInstance.tooltip)) {
        return;
      }
      chartInstance.tooltip.fadeOut();
    },
    tooltip: {},
    renderTooltip: function renderTooltip(tooltipOptions, elementId, dataId) {
    ...
    destroy: function destroy(dataId) {
      var currentInstance = window.sfBlazor.getCompInstance(dataId);
      if (dataId && currentInstance) {
        currentInstance.unWireEvents();
      }
    }
  };
  return SmithChart;
...
}]);(async()=>{await import(`${document.baseURI}_content/Syncfusion.Blazor/scripts/syncfusion-blazor-base.min.js?v=19.3.1`).then(()=>{sfBlazor.loadDependencies('sfsmithchart');})})();