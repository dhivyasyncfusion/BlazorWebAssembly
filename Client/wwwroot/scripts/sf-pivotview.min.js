(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["sf-pivotview"],{
...
window.sfBlazor.PivotView = function () {
  ...
  var RADIOBTN = 'e-adaptive-calc-radio-btn';
  var PIVOT_CELL_CONTAINER = 'e-pivotcell-container';
  var GRID_PAGER = "pivot-pager-container";
  ...
  var PivotButton = /** @class */function () {
    ...
    PivotButton.prototype.createDragClone = function (field, isTreeElement, args) {
      ...
      contentElement.innerText = field ? field.caption ? field.caption : field.name : '';
  ...
  var GroupingBar = /** @class */function () {
    ...
    GroupingBar.prototype.refreshUI = function () {
      ...
      if (groupingTable) {
        ...
        //groupingTable.style.width = (select('#' + this.parent.element.id + '_grid', this.parent.element) as any).offsetWidth + 'px';
        ...
        if (sf.base.select('#' + this.parent.element.id + '_grid', this.parent.element)) {
          var emptyRowHeader = this.parent.element.querySelector('.' + HEADERCONTENT).querySelector('.' + COLUMN_HEADER);
          emptyRowHeader.querySelector('.' + HEADERCELL).classList.add(GROUP_ROW);
          emptyRowHeader.style.height = 'auto';
          ...
          // if (this.parent.element.querySelector('.' + cls.HEADERCONTENT).querySelector('.' + cls.RESIZE_HANDLER)) {
          //     emptyRowHeader.style.height = (this.parent.element.querySelector('.' + cls.HEADERCONTENT).querySelector('.' + cls.RESIZE_HANDLER) as HTMLElement).style.height = groupHeight + 'px';
          // }
          /* eslint-enable */
          ...
          //         if (!closest(handlerElement, '.' + cls.FREEZED_CELL)) {
  ...
  var CalculatedField = /** @class */function () {
    ...
    CalculatedField.prototype.getNodeLocation = function (treeElement, id) {
      var position = treeElement.querySelector('li[data-uid=' + '\"' + CSS.escape(JSON.parse(id)) + '\"' + ']').getBoundingClientRect(); /* eslint-disable-line */
    ...
    CalculatedField.prototype.updateAccordionLabel = function (target) {
      ...
      var aggregateValue = sf.base.closest(eventTarget, '.' + RADIOBTN).querySelector('input');
      sf.base.closest(eventTarget, '.' + ACCORD_ITEM).querySelector('.' + LABEl).innerText = field + ' (' + type + ')';
      sf.base.closest(eventTarget, '.' + ACCORD_ITEM).querySelector('[data-type').setAttribute('data-type', aggregateValue.getAttribute('data-value'));
    };
      ...
      if (element.querySelector('.' + FORMULA)) {
        ...
        for (var i = 0; i < li.length; i++) {
          ...
          if (this.parent.options.dataType === 'olap') {
            ...
            if (!li[i].querySelector('.' + TEXT_CONTENT_CLASS + ' span.e-drag') && li[i].querySelector('.' + TEXT_CONTENT_CLASS + ' span')) {
    ...
    CalculatedField.prototype.onDragging = function (e) {
      var clonedNode = sf.base.select('#' + this.parent.element.id + '_DragClone ');
    ...
    CalculatedField.prototype.calculatedDragClone = function (args) {
      ...
      contentElement.innerText = sf.base.closest(args.element, 'li').getAttribute('data-caption');
      cloneElement.appendChild(contentElement);
      document.body.appendChild(cloneElement);
      return cloneElement;
    };
  ...
  var TreeRenderer = /** @class */function () {
    ...
    TreeRenderer.prototype.updateFieldListIcons = function (id, dragText) {
      if (this.parent.fieldList) {
        var treeElement = document.getElementById(id);
        var liElement = [].slice.call(treeElement.querySelectorAll('ul li'));
        for (var i = 0; i < liElement.length; i++) {
          var nodeDataId = liElement[i].getAttribute('data-uid');
          this.updateTreeNode(liElement[i], nodeDataId, dragText);
        }
      }
    };
    TreeRenderer.prototype.updateTreeNode = function (node, nodeDataId, dragText) {
      var allowDrag = false;
      if (this.parent.options.dataType === 'olap') {
      ...
      if (allowDrag && !this.parent.isAdaptive) {
        var field = this.parent.commonActionModule.getFieldInfo(nodeDataId);
        allowDrag = false;
        var dragElement = sf.base.createElement('span', {
          attrs: {
            'tabindex': '-1',
        ...
        if (node.querySelector('.' + CHECKBOX_CONTAINER) && !node.querySelector('.' + DRAG_CLASS) && liTextElement) {
          liTextElement.appendChild(dragElement);
        }
  ...
  var SfPivotFieldList = /** @class */function () {
    function SfPivotFieldList(dataId, element, options, dotnetRef) {
      window.sfBlazor = window.sfBlazor;
    ...
    SfPivotFieldList.prototype.getOptions = function (element, options) {
      ...
      this.pivotGridModule = options.pivotGridModule ? window.sfBlazor.getCompInstance(options.pivotGridModule) : this.pivotGridModule;
      if (this.pivotGridModule && sf.base.isNullOrUndefined(this.pivotGridModule.pivotFieldListModule)) {
        this.pivotGridModule.pivotFieldListModule = this;
      }
      this.fieldList = options.fieldList ? options.fieldList : this.fieldList;
      ...
      if (this.parentElement && sf.base.select('#' + this.parentElement.id + '_title', this.parentElement)) {
        sf.base.setStyleAttribute(sf.base.select('#' + this.parentElement.id + '_title', this.parentElement), {
          'width': options.allowDeferLayoutUpdate && this.isAdaptive ? 'auto' : '100%'
        });
      }
    };
    ...
    SfPivotFieldList.prototype.updateFieldList = function (containerElement) {
      var footer = containerElement.querySelectorAll('.' + FOOTER_CONTENT_CLASS)[1];
      if (sf.base.select('#' + containerElement.id + '_dialog-content', containerElement)) {
        ...
        if (footer && footer.querySelector('.e-field-list-footer-content')) {
    ...
    SfPivotFieldList.prototype.destroy = function (isDestroyed) {
      this.unWireEvents();
  ...
  var CommonKeyboardInteraction = /** @class */function () {
    ...
    CommonKeyboardInteraction.prototype.processFilterNodeSelection = function (e) {
      ...
      if (target && sf.base.closest(target, '.' + SELECT_ALL_CLASS) && e.keyCode === 40) {
        ...
        if (memberEditorTree && memberEditorTree.querySelector('li')) {
          ...
          if (firstLi) {
            sf.base.removeClass([firstLi], [NODE_FOCUS]);
          }
          sf.base.addClass([firstLi], [NODE_FOCUS]);
          firstLi.focus();
          e.stopPropagation();
          e.preventDefault();
          return;
        ...
        if (memberEditorTree.querySelector('li' + '.' + NODE_FOCUS) && memberEditorTree.querySelector('li') && memberEditorTree.querySelector('li') === memberEditorTree.querySelector('li' + '.' + NODE_FOCUS)) {
    ...
    CommonKeyboardInteraction.prototype.processClose = function (e) {
      ...
      if (target && sf.base.closest(target, '.' + POPUP + '.' + POPUP_OPEN)) {
        ...
        /* eslint-enable */
        var pivot_1 = this; /* eslint-disable-line */
  ...
  var ActionBase = /** @class */function () {
    ...
    ActionBase.prototype.getElementInfo = function () {
      if (this.parent instanceof SfPivotView) {
        ...
        this.parent.dotNetRef.invokeMethodAsync('GetElementInfo', domInfo.width, domInfo.height, this.parent.element.clientWidth, this.parent.element.clientHeight);
      }
    };
    ActionBase.prototype.updateActiveNode = function (treeElement, id) {
      ...
      sf.base.addClass([treeElement.querySelector('li[data-uid=' + '\"' + id + '\"' + ']')], PREV_ACTIVE_NODE); /* eslint-disable-line */
    ...
    ActionBase.prototype.isFullRowElement = function (top, left) {
      ...
      var isFullRow = target.classList.contains(FULL_ROW);
      return JSON.stringify(this.parent instanceof SfPivotFieldList && sf.base.closest(target, '.e-field-list-tree-outer-div') ? isFullRow && !sf.base.isNullOrUndefined(target.parentElement.querySelector('.' + DRAG_CLASS)) : isFullRow);
    };
    ...
    ActionBase.prototype.updateDropIndicator = function (e) {
      ...
      } else if (this.parent instanceof SfPivotFieldList && (!this.parent.isDragging || !e.target.classList.contains(DROPPABLE_CLASS) && e.type === 'mouseleave')) {
        sf.base.removeClass([].slice.call(this.parent.parentElement.querySelectorAll('.' + DROP_INDICATOR_CLASS)), INDICATOR_HOVER_CLASS);
        sf.base.removeClass([].slice.call(this.parent.parentElement.querySelectorAll('.' + DROP_INDICATOR_CLASS + '-last')), INDICATOR_HOVER_CLASS);
      }
  ...
  var Selection = /** @class */function () {
    ...
    Selection.prototype.mouseClickHandler = function (e) {
      ...
      if ((target.classList.contains(HEADERCELL) || target.classList.contains(HEADER_CELL_DIV) || target.classList.contains(ROWSHEADER) || target.classList.contains(ROW_CELL_CLASS) || target.classList.contains(STACKED_HEADER_CELL_DIV) || target.classList.contains(HEADER_TEXT) || target.classList.contains(STACKED_HEADER_TEXT)) && this.parent.options.dataType === 'pivot') {
        ...
        if (target.classList.contains(HEADERCELL) || target.classList.contains(ROWSHEADER) || target.classList.contains(ROW_CELL_CLASS)) {
          ...
          targetElement = target.parentElement.parentElement; /* eslint-disable-line */
        }
    ...
    Selection.prototype.cellClicked = function (target, e) {
      var _this = this;
      var targetElement = null;
      if (target.classList.contains(HEADERCELL) || target.classList.contains(STACKED_HEADER_CELL) || target.classList.contains(STACKED_HEADER_CELL_DIV) || target.classList.contains(STACKED_HEADER_TEXT) || target.classList.contains(PIVOT_CELL_CONTAINER) || target.classList.contains(HEADER_TEXT) || target.classList.contains(HEADER_CELL_DIV)) {
        targetElement = sf.base.closest(target, 'th.' + HEADERCELL);
      } else if (target.classList.contains(CELLVALUE) || target.classList.contains(ROW_CELL_CLASS)) {
        targetElement = sf.base.closest(target, 'td.' + ROW_CELL_CLASS);
          ...
          if (this.parent.gridSettings.selectionSettings.mode === 'Both' ? !targetElement.classList.contains(ROW_CELL_CLASS) : this.parent.gridSettings.selectionSettings.mode === 'Cell' ? targetElement.classList.contains(COLUMNSHEADER) : this.parent.gridSettings.selectionSettings.mode !== 'Row') {
            this.parent.dotNetRef.invokeMethodAsync('ColumnSelectHandler', rowIndex_1, colIndex_1, e).then(function (cancel) {
              ...
              if (!cancel) {
                ...
                _this.afterColumnSelected(targetElement, e, colIndex_1, rowIndex_1);
              }
            });
          } else {
            this.clearSelection(targetElement, e, colIndex_1, rowIndex_1);
            this.afterColumnSelected(targetElement, e, colIndex_1, rowIndex_1);
          }
        }
        if (!this.parent.gridSettings.allowSelection && !(target.classList.contains(EXPAND_ICON) || target.classList.contains(COLLAPSE_ICON))) {
          /* eslint-disable-line */
          this.parent.dotNetRef.invokeMethodAsync('CellClickedHandler', rowIndex_1, colIndex_1, e); /* eslint-disable-line */
        }
      }
    };
    Selection.prototype.afterColumnSelected = function (targetElement, e, colIndex, rowIndex) {
      this.getSelectedCellsPos();
      var cellPos = JSON.stringify(this.savedSelectedCellsPos);
      if (this.parent.gridSettings.selectionSettings.mode === 'Both' ? !targetElement.classList.contains(ROW_CELL_CLASS) : this.parent.gridSettings.selectionSettings.mode === 'Cell' ? targetElement.classList.contains(COLUMNSHEADER) : this.parent.gridSettings.selectionSettings.mode !== 'Row') {
        this.parent.dotNetRef.invokeMethodAsync('CellClickedHandler', rowIndex, colIndex, e); /* eslint-disable-line */
  ...
  var VirtualScroll = /** @class */function () {
    function VirtualScroll(parent) {
      ...
      this.isFireFox = sf.base.Browser.userAgent.toLowerCase().indexOf('firefox') > -1;
      this.parent = parent;
    ...
    VirtualScroll.prototype.wireEvents = function () {
      ...
      if (this.parent.displayOptions.view !== 'Chart') {
        ...
        var gridContent = this.parent.element.querySelector('.' + GRIDCONTENT_DIV);
        ...
        if (this.isFireFox) {
          sf.base.EventHandler.clearEvents(mHdr);
        }
        var ele = this.parent.isAdaptive ? mCont : gridContent.querySelector('.' + VIRTUALTABLE_DIV);
        sf.base.EventHandler.add(ele, 'scroll touchmove pointermove', this.onHorizondalScroll(mHdr, mCont, mCont), this);
        sf.base.EventHandler.add(mCont.parentElement, 'scroll wheel touchmove pointermove keyup keydown', this.onVerticalScroll(mCont, mCont.parentElement), this);
        if (this.isFireFox) {
          sf.base.EventHandler.add(ele, 'mouseup touchend scroll', this.common(mHdr, mCont, mCont), this);
          if (!this.parent.isAdaptive) {
            sf.base.EventHandler.add(mCont.parentElement, 'mouseup touchend scroll', this.common(mHdr, mCont.parentElement, mCont), this);
          }
        } else {
          sf.base.EventHandler.add(ele, 'mouseup touchend', this.common(mHdr, mCont, mCont), this);
          if (!this.parent.isAdaptive) {
            sf.base.EventHandler.add(mCont.parentElement, 'mouseup touchend', this.common(mHdr, mCont.parentElement, mCont), this);
          }
        }
        ...
        sf.base.EventHandler.add(mHdr, 'touchmove pointermove', this.onTouchScroll(mHdr, mCont, mCont), this);
        sf.base.EventHandler.add(mCont, 'touchstart pointerdown', this.setPageXY(), this);
        sf.base.EventHandler.add(mCont, 'touchmove pointermove', this.onTouchScroll(mHdr, mCont, mCont), this);
      }
    };
    VirtualScroll.prototype.unWireEvents = function () {
      if (this.parent.displayOptions.view !== 'Chart') {
        ...
        var mScrollBar = gridContent.querySelector('.' + VIRTUALTABLE_DIV);
        var ele = this.parent.isAdaptive ? mCont : gridContent.querySelector('.' + VIRTUALTABLE_DIV);
        sf.base.EventHandler.remove(ele, 'scroll touchmove pointermove', this.onHorizondalScroll(mHdr, mCont, mCont));
        sf.base.EventHandler.remove(mCont.parentElement, 'scroll wheel touchmove pointermove keyup keydown', this.onVerticalScroll(mCont, mCont.parentElement));
        if (this.isFireFox) {
          sf.base.EventHandler.remove(ele, 'mouseup touchend scroll', this.common(mHdr, mCont, mCont));
          if (!this.parent.isAdaptive) {
            sf.base.EventHandler.remove(mCont.parentElement, 'mouseup touchend scroll', this.common(mHdr, mCont.parentElement, mCont));
          }
        } else {
          sf.base.EventHandler.remove(ele, 'mouseup touchend', this.common(mHdr, mCont, mCont));
          if (!this.parent.isAdaptive) {
            sf.base.EventHandler.remove(mCont.parentElement, 'mouseup touchend', this.common(mHdr, mCont.parentElement, mCont));
          }
        }
        ...
        sf.base.EventHandler.remove(mHdr, 'touchmove pointermove', this.onTouchScroll(mHdr, mCont, mCont));
        sf.base.EventHandler.remove(mCont, 'touchstart pointerdown', this.setPageXY());
        sf.base.EventHandler.remove(mCont, 'touchmove pointermove', this.onTouchScroll(mHdr, mCont, mCont));
      }
    };
    ...
    VirtualScroll.prototype.onCustomScrollbarScroll = function (mCont, mHdr) {
      ...
      var header = mHdr;
      return function (e) {
        var eContent = _this.parent.element.querySelector('.' + GRID_CONTENT);
    ...
    VirtualScroll.prototype.onTouchScroll = function (mHdr, mCont, fCont) {
      ...
      return function (e) {
        ...
        var top = _this.parent.element.querySelector('.' + GRID_CLASS + ' .' + GRID_CONTENT).scrollTop + (_this.pageXY.y - pageXY.y);
        var ele = _this.parent.isAdaptive ? mCont : sf.base.closest(element, '.' + GRIDCONTENT_DIV).querySelector('.' + VIRTUALTABLE_DIV);
    ...
    VirtualScroll.prototype.update = function (mHdr, mCont, fCont, top, left, e, ele) {
      var _this = this;
      ...
      if (this.parent.pageSettings) {
        if (this.direction === 'vertical') {
          ...
          var exactSize_1 = this.parent.pageSettings.rowPageSize * rowValues_1 * this.parent.gridSettings.rowHeight;
          var section = Math.ceil(top / exactSize_1);
          section += this.parent.options.enableOptimizedRendering && vScrollPos <= 0 ? 1 : 0;
          if (this.parent.scrollPosObject.vertical === section || this.parent.pageSettings.rowPageSize >= this.parent.scrollPageInfo.rowCount) {
            if (this.parent.options.enableOptimizedRendering && this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)) {
              sf.base.removeClass([fCont], ['e-virtual-pivot-content']);
              sf.base.addClass([this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)], [PIVOT_HIDE_LOADER]);
            }
            return;
          }
          this.parent.scrollPosObject.vertical = section;
          this.parent.pageSettings.currentRowPage = section > 1 ? section : 1;
          var rowStartPos_1 = 0;
          if (this.parent.options.enableOptimizedRendering && this.parent.internalGrid && this.parent.internalGrid.element.querySelector('.e-spinner-inner')) {
            sf.base.addClass([this.parent.internalGrid.element.querySelector('.e-spinner-inner')], [PIVOT_HIDE_LOADER]);
          }
          this.parent.dotNetRef.invokeMethodAsync('UpdateScrollInfo', section, this.direction).then(function (scrollPageInfo) {
            ...
            var exactpage = Math.ceil(rowStartPos_1 / (_this.parent.pageSettings.rowPageSize * rowValues_1));
            ...
            if (_this.parent.options.enableOptimizedRendering && _this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)) {
              sf.base.removeClass([fCont], ['e-virtual-pivot-content']);
              sf.base.addClass([_this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)], [PIVOT_HIDE_LOADER]);
              if (_this.parent.internalGrid && _this.parent.internalGrid.element.querySelector('.e-spinner-inner')) {
                sf.base.removeClass([_this.parent.internalGrid.element.querySelector('.e-spinner-inner')], [PIVOT_HIDE_LOADER]);
              }
            }
          });
        } else {
          ...
          var exactSize_2 = this.parent.pageSettings.columnPageSize * colValues_1 * this.parent.gridSettings.columnWidth;
          var section = Math.ceil(Math.abs(left) / exactSize_2);
          section += this.parent.options.enableOptimizedRendering && hScrollPos <= 0 ? 1 : 0;
          if (this.parent.scrollPosObject.horizontal === section) {
            if (this.parent.options.enableOptimizedRendering && this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)) {
              sf.base.removeClass([mHdr, mCont], ['e-virtual-pivot-content']);
              sf.base.addClass([this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)], [PIVOT_HIDE_LOADER]);
            }
            return;
          }
          this.parent.scrollPosObject.horizontal = section;
          this.parent.pageSettings.currentColumnPage = section > 1 ? section : 1;
          var colStartPos_1 = 0;
          if (this.parent.options.enableOptimizedRendering && this.parent.internalGrid && this.parent.internalGrid.element.querySelector('.e-spinner-inner')) {
            sf.base.addClass([this.parent.internalGrid.element.querySelector('.e-spinner-inner')], [PIVOT_HIDE_LOADER]);
          }
          this.parent.dotNetRef.invokeMethodAsync('UpdateScrollInfo', section, this.direction).then(function (scrollPageInfo) {
            ...
            var exactPage = Math.ceil(colStartPos_1 / (_this.parent.pageSettings.columnPageSize * colValues_1));
    ...
    VirtualScroll.prototype.common = function (mHdr, mCont, fCont) {
      var _this = this;
      return function (e) {
        var ele = _this.parent.isAdaptive ? mCont : sf.base.closest(mCont, '.' + GRIDCONTENT_DIV).querySelector('.' + VIRTUALTABLE_DIV);
        if (_this.parent.options.enableOptimizedRendering) {
          if (_this.direction === 'vertical') {
            if (_this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)) {
              sf.base.addClass([fCont], ['e-virtual-pivot-content']);
              sf.base.removeClass([_this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)], [PIVOT_HIDE_LOADER]);
            }
          } else {
            if (_this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)) {
              sf.base.addClass([mHdr, mCont], ['e-virtual-pivot-content']);
              sf.base.removeClass([_this.parent.element.querySelector('.' + PIVOT_CONTENT_LOADER)], [PIVOT_HIDE_LOADER]);
            }
          }
        }
        _this.update(mHdr, mCont, fCont, _this.parent.element.querySelector('.' + GRID_CLASS + ' .' + GRID_CONTENT).scrollTop * _this.parent.verticalScrollScale, ele.scrollLeft * _this.parent.horizontalScrollScale, e, ele);
      };
    };
    VirtualScroll.prototype.onHorizondalScroll = function (mHdr, mCont, fCont) {
      ...
      var timeOutObj;
      var ele = this.parent.isAdaptive ? mCont : sf.base.closest(mCont, '.' + GRIDCONTENT_DIV).querySelector('.' + VIRTUALTABLE_DIV);
      ...
      horiOffset = this.parent.options.enableRtl ? horiOffset : -horiOffset;
      ...
      return function (e) {
        ...
        if (e.type === 'wheel' || e.type === 'touchmove' || _this.eventType === 'wheel' || _this.eventType === 'touchmove') {
          ...
          timeOutObj = setTimeout(function () {
            ...
            _this.update(mHdr, mCont, fCont, mCont.parentElement.scrollTop * _this.parent.verticalScrollScale, left, e, ele);
          }, 300);
        }
        ...
        horiOffset = _this.parent.options.enableRtl ? horiOffset : -horiOffset;
        ...
        if (eleScrollLeft < _this.parent.scrollerBrowserLimit) {
          ...
          for (var i = 0, j = _this.parent.element.querySelectorAll('.' + FREEZED_CELL); i < j.length; i++) {
            if (_this.parent.options.enableRtl) {
              j[i].style.right = Number(horiOffset) + 'px';
            } else {
              j[i].style.left = Number(-horiOffset) + 'px';
            }
        ...
        if (_this.parent.scrollPosObject.horizontalSection > left ? true : excessMove > 1 && notLastPage) {
          ...
          horiOffset = -(left - (_this.parent.scrollPosObject.horizontalSection + excessMove) - sf.base.closest(mCont, '.' + GRIDCONTENT_DIV).querySelector('.' + VIRTUALTABLE_DIV).scrollLeft);
    ...
    VirtualScroll.prototype.onVerticalScroll = function (fCont, mCont) {
      ...
      return function (e) {
        ...
        if (e.type === 'wheel' || e.type === 'touchmove' || _this.eventType === 'wheel' || _this.eventType === 'touchmove' || e.type === 'keyup' || e.type === 'keydown') {
          var ele_1 = _this.parent.isAdaptive ? mCont : sf.base.closest(mCont, '.' + GRIDCONTENT_DIV).querySelector('.' + VIRTUALTABLE_DIV);
          clearTimeout(timeOutObj);
          timeOutObj = setTimeout(function () {
            var scrollLeft = 0;
            ...
            _this.update(null, mCont, fCont, mCont.scrollTop * _this.parent.verticalScrollScale, scrollLeft * _this.parent.horizontalScrollScale, e, ele_1);
          }, 300);
        }
        /* eslint-enable */
        ...
        var excessMove = _this.parent.scrollPosObject.verticalSection > top ? -(_this.parent.scrollPosObject.verticalSection - top) : top + mCont.clientHeight - (_this.parent.scrollPosObject.verticalSection + fCont.querySelector('.' + TABLE).offsetHeight);
        ...
        if (_this.parent.scrollPosObject.verticalSection > top ? true : excessMove > 1 && notLastPage) {
          ...
          var movableTable = _this.parent.element.querySelector('.' + GRID_CONTENT).querySelector('.' + TABLE);
    ...
    VirtualScroll.prototype.updateScrollInfo = function (element, columnCount, rowCount) {
      if (this.parent.options.enableVirtualization) {
        var horiScrollHeight = this.parent.getScrollBarWidth();
        var mCnt = element.querySelector('.' + CONTENT_VIRTUALTABLE_DIV) ? element.querySelector('.' + CONTENT_VIRTUALTABLE_DIV) : element.querySelector('.' + GRID_CONTENT);
        var fCnt = element.querySelector('.' + FROZENCONTENT_DIV);
        var mHdr = element.querySelector('.' + MOVABLEHEADER_DIV);
        var gridContentDiv = element.querySelector('.' + GRIDCONTENT_DIV);
        var isHorizontalOverFlow = mCnt.parentElement.offsetWidth < mCnt.querySelector('.' + TABLE).offsetWidth;
        if (this.parent.options.enableOptimizedRendering) {
          var loaderElement = element.querySelector('.' + PIVOT_CONTENT_LOADER);
          if (!loaderElement) {
            loaderElement = sf.base.createElement('div', {
              className: PIVOT_CONTENT_LOADER + ' ' + PIVOT_HIDE_LOADER,
              innerHTML: this.parent.options.localeObj.loading
            });
            if (this.parent.internalGrid && this.parent.internalGrid.element && !element.querySelector('.' + PIVOT_CONTENT_LOADER)) {
              this.parent.internalGrid.element.insertBefore(loaderElement, this.parent.internalGrid.element.firstElementChild);
            }
          }
          if (element.querySelector('.' + HEADERCONTENT) && element.querySelector('.' + GRIDCONTENT_DIV)) {
            var height = element.querySelector('.' + GRIDCONTENT_DIV).offsetHeight + element.querySelector('.' + HEADERCONTENT).offsetHeight;
            sf.base.setStyleAttribute(loaderElement, {
              'height': height + 'px',
              'width': element.querySelector('.' + HEADERCONTENT).offsetWidth + 'px',
              'padding': Math.floor(height / 2) + 'px'
            });
          }
        }
        if (element.querySelector('.' + GRID_CONTENT) && !element.querySelector('.' + GRID_CONTENT).querySelector('.' + VIRTUALTRACK_DIV)) {
          this.virtualDiv = sf.base.createElement('div', {
            className: VIRTUALTRACK_DIV
          });
          element.querySelector('.' + GRID_CONTENT).appendChild(this.virtualDiv);
        }
        ...
        if (gridContentDiv && !virtualScrollTableDiv) {
          this.virtualTableDiv = sf.base.createElement('div', {
          ...
          this.virtualScrollDiv = sf.base.createElement('div', {
            className: VIRTUALSCROLL_DIV
          });
          virtualScrollTableDiv.appendChild(this.virtualScrollDiv);
          sf.base.setStyleAttribute(virtualScrollTableDiv, {
            minHeight: horiScrollHeight + 'px',
            maxHeight: horiScrollHeight + 'px',
            overflowX: 'scroll'
          });
          sf.base.setStyleAttribute(this.virtualScrollDiv, {
            height: '0.1px'
          });
          var newContentVirtualDiv = sf.base.createElement('div', {
            className: CONTENT_VIRTUALTABLE_DIV
          });
          newContentVirtualDiv.append.apply(newContentVirtualDiv, mCnt.childNodes);
          mCnt.appendChild(newContentVirtualDiv);
          newContentVirtualDiv.scrollLeft = 0;
        }
        var movableTable = element.querySelector('.' + GRID_CONTENT).querySelector('.' + TABLE);
        ...
        var ele = this.parent.isAdaptive ? mCnt : gridContentDiv.querySelector('.' + VIRTUALTABLE_DIV) ? gridContentDiv.querySelector('.' + VIRTUALTABLE_DIV) : mCnt;
        var verOffset = mCnt.parentElement.scrollTop > this.parent.scrollerBrowserLimit || this.direction === 'horizondal' ? mCnt.querySelector('.' + TABLE).style.transform.split(',')[1].trim() : (this.parent.options.enableOptimizedRendering ? mCnt.parentElement.scrollTop : -(mCnt.parentElement.scrollTop * this.parent.verticalScrollScale - this.parent.scrollPosObject.verticalSection - mCnt.parentElement.scrollTop)) + 'px)';
        ...
        // var horiOffset = ele.scrollLeft > this.parent.scrollerBrowserLimit ? mCnt.querySelector('.' + TABLE).style.transform.split(',')[0].trim() + ',' : 'translate(' + ele.scrollLeft + 'px,'; //'translate(' + -(ele.scrollLeft * this.parent.horizontalScrollScale - this.parent.scrollPosObject.horizontalSection - ele.scrollLeft) + 'px,';
        // var horiOffset = ele.scrollLeft > this.parent.scrollerBrowserLimit ? mCnt.querySelector('.' + TABLE).style.transform.split(',')[0].trim() + ',' : 'translate(' + (this.parent.options.enableOptimizedRendering ? ele.scrollLeft : -(ele.scrollLeft * this.parent.horizontalScrollScale - this.parent.scrollPosObject.horizontalSection - ele.scrollLeft)) + 'px,';
        // let verOffset: string = (mCnt.parentElement.scrollTop > this.parent.scrollerBrowserLimit) ?
        //     (mCnt.querySelector('.' + TABLE) as HTMLElement).style.transform.split(',')[1].trim() :
        //     -(((mCnt.parentElement.scrollTop * this.parent.verticalScrollScale) - this.parent.scrollPosObject.verticalSection - mCnt.parentElement.scrollTop)) + 'px)';
        var eleScrollLeft = Math.abs(ele.scrollLeft);
        var horiOffset = eleScrollLeft > this.parent.scrollerBrowserLimit ? mCnt.querySelector('.' + TABLE).style.transform.split(',')[0].trim() + ',' : 'translate(' + (this.parent.options.enableRtl ? 1 : -1) * (eleScrollLeft * this.parent.horizontalScrollScale - this.parent.scrollPosObject.horizontalSection - eleScrollLeft) + 'px,';
        ...
        if (!isHorizontalOverFlow && gridContentDiv && virtualScrollTableDiv || this.parent.isAdaptive) {
        ...
        for (var i = 0, j = element.querySelectorAll('.' + FREEZED_CELL); i < j.length; i++) {
          if (this.parent.options.enableRtl) {
            j[i].style.right = -translateValue + 'px';
          } else {
            j[i].style.left = translateValue + 'px';
          }
        }
        if (isHorizontalOverFlow) {
          ...
          if (this.parent.options.height !== 'auto') {
            element.querySelector('.' + VIRTUALSCROLL_DIV).style.width = newScrollWidth + horiScrollHeight + 'px';
          } else {
            element.querySelector('.' + VIRTUALSCROLL_DIV).style.width = newScrollWidth + 'px';
          }
        }
        ...
        var exactSize = this.parent.pageSettings.columnPageSize * colValues * this.parent.options.gridSettings.columnWidth;
        ...
        if (this.parent && exactSize > 0 && hScrollPos <= exactSize && ele.scrollLeft > 0) {
          ...
          if (mCntScrollPos > 1) {
            ...
            if (this.parent.options.height !== 'auto') {
              this.parent.scrollPosObject.horizontalSection -= horiScrollHeight;
            }
            this.parent.scrollPosObject.horizontalSection = this.parent.scrollPosObject.horizontalSection >= -element.querySelector('.' + GRID_CLASS).querySelector('.' + HEADERCELL + '.' + FREEZED_CELL).offsetWidth ? this.parent.scrollPosObject.horizontalSection : -element.querySelector('.' + GRID_CLASS).querySelector('.' + HEADERCELL + '.' + FREEZED_CELL).offsetWidth;
          }
          ...
          // horiOffset = ele.scrollLeft > this.parent.scrollerBrowserLimit ? mCnt.querySelector('.' + TABLE).style.transform.split(',')[0].trim() + ',' : 'translate(' + (this.parent.options.enableOptimizedRendering ? ele.scrollLeft : -(ele.scrollLeft * this.parent.horizontalScrollScale - this.parent.scrollPosObject.horizontalSection - ele.scrollLeft)) + 'px,';
          sf.base.setStyleAttribute(mCnt.querySelector('.' + TABLE), {
            transform: horiOffset + verOffset
          });
          sf.base.setStyleAttribute(mHdr.querySelector('.' + TABLE), {
        ...
        if (element.querySelector('.' + VIRTUALSCROLL_DIV)) {
          var scrollbarWidth = element.querySelector('.' + VIRTUALSCROLL_DIV).style.width;
          if (!(scrollbarWidth && Number(scrollbarWidth.replace('px', '')) > 0)) {
            sf.base.closest(element.querySelector('.' + VIRTUALSCROLL_DIV), '.' + VIRTUALTABLE_DIV).style.display = 'none';
          }
        }
        if (this.parent.options.isEmptyData) {
          mCnt.style.height = (movableTable ? movableTable.querySelector('tr').offsetHeight : 30) + 'px';
        }
        if (this.parent.virtualScrollModule && !this.parent.options.isEmptyData) {
          this.onHorizondalScroll(mHdr, mCnt, mCnt);
        }
      }
    };
  ...
  var SfPivotView = /** @class */function () {
    function SfPivotView(dataId, element, options, dotnetRef) {
      this.scrollPageInfo = {
        rowCount: 0,
        columnCount: 0,
        colFirstLvl: 0,
    ...
    SfPivotView.prototype.getOptions = function (element, options) {
      ...
      this.isAdaptive = sf.base.Browser.isDevice;
      this.internalGrid = options.internalGrid ? window.sfBlazor.getCompInstance(options.internalGrid) : this.internalGrid;
      this.pivotFieldListModule = options.fieldListModule ? window.sfBlazor.getCompInstance(options.fieldListModule) : this.pivotFieldListModule;
      this.fieldList = options.fieldList ? options.fieldList : this.fieldList;
      this.dataSourceSettings = options.dataSourceSettings;
    ...
    SfPivotView.prototype.contentReady = function () {
      ...
      if (this.options.renderGrid) {
        if (this.displayOptions.view == 'Both' && (this.options.showToolbar ? this.displayOptions.primary == 'Table' : true) || this.displayOptions.view == 'Table') {
          this.updateGridUI(this.element, this.options, this.scrollPageInfo.columnCount, this.scrollPageInfo.rowCount);
        } else if (this.options.showToolbar && this.displayOptions.view == 'Both' && this.displayOptions.primary == 'Chart' || this.displayOptions.view == 'Chart') {
          if (this.options.showGroupingBar && this.groupingBarModule) {
            this.groupingBarModule.refreshUI();
          }
        }
        if (!(this.options.showGroupingBar && this.groupingBarModule) && this.element && this.element.querySelectorAll('.e-axis-container').length > 0) {
    ...
    SfPivotView.prototype.getSelectedCellDom = function (domElement) {
      if (domElement) {
        return JSON.stringify(window.sfBlazor.getDomObject('currentCell', domElement)); /* eslint-disable-line */
      } else {
        return null;
      }
    };
    ...
    SfPivotView.prototype.exportDocument = function (filename, bytesBase64) {
    ...
    SfPivotView.prototype.getSelectedCells = function () {
      ...
      for (var _i = 0, selectedElements_1 = selectedElements; _i < selectedElements_1.length; _i++) {
        ...
        if (!sf.base.isNullOrUndefined(colIndex) && !sf.base.isNullOrUndefined(rowIndex)) {
          cellInfo['index'] = rowIndex; /* eslint-disable-line */
          cellInfo['aria-colindex'] = colIndex;
          domCollection.push(cellInfo);
        }
      }
    ...
    SfPivotView.prototype.mouseClickHandler = function (e) {
      ...
      var target = e.target;
      if (this.enableValueSorting && this.options.dataType === 'pivot' && (target.classList.contains(SORT_FILTER_DIV) || target.classList.contains(STACKED_HEADER_TEXT) || target.classList.contains(STACKED_HEADER_CELL_DIV) || target.classList.contains(HEADER_TEXT) || target.classList.contains(STACKED_HEADER_CELL_DIV) || target.classList.contains(HEADER_CELL_DIV) || target.classList.contains(HEADERCELL) || target.classList.contains(CELLVALUE) || target.classList.contains(COLUMNSHEADER) || target.classList.contains(ROWSHEADER) || target.classList.contains(PIVOT_CELL_CONTAINER))) {
        var element = sf.base.closest(target, 'td.' + ROWSHEADER + ',th.' + COLUMNSHEADER);
        if (element && (sf.base.closest(element, '.' + HEADERCONTENT) && this.dataSourceSettings.valueAxis === 'column' || sf.base.closest(element, 'tr.' + ROW) && this.dataSourceSettings.valueAxis === 'row' && (element.classList.contains(ROWSHEADER) || element.classList.contains(SUBTOTAL)))) {
          if (this.enableValueSorting) {
            ...
            if (this.dataSourceSettings.valueAxis === 'column' && (this.dataSourceSettings.values.length > 1 || this.dataSourceSettings.alwaysShowValueHeader)) {
              colIndex = Number(element.getAttribute('aria-colindex')) + Number(element.getAttribute('colspan')) - 1;
            }
    ...
    SfPivotView.prototype.getHeightAsNumber = function () {
      ...
      if (height < this.getminHeightAsNumber()) {
        height = this.getminHeightAsNumber();
      }
      return height;
    };
    SfPivotView.prototype.getminHeightAsNumber = function () {
      var height;
      if (isNaN(this.options.minHeight)) {
        if (this.options.minHeight.toString().indexOf('%') > -1) {
          height = parseFloat(this.options.minHeight.toString()) / 100 * this.element.offsetHeight;
        } else if (this.options.minHeight.toString().indexOf('px') > -1) {
          height = Number(this.options.minHeight.toString().split('px')[0]);
    ...
    SfPivotView.prototype.calculateGridHeight = function (elementCreated, rowCount, columnCount) {
      ...
      if (isNaN(parHeight)) {
        parHeight = parHeight > this.getminHeightAsNumber() ? parHeight : this.getminHeightAsNumber();
      }
      var domInfo = this.element.getBoundingClientRect();
      var cellInfo = {
        width: domInfo.width.toString(),
        height: domInfo.height.toString(),
        clientWidth: this.element.clientWidth.toString(),
        clientHeight: this.element.clientHeight.toString(),
        gridHeight: gridHeight ? gridHeight.toString() : gridHeight
      };
      if (this.gridSettings.height === 'auto' && parHeight && this.element.querySelector('.' + GRID_HEADER)) {
        var colHeaderHeight = this.element.querySelector('.' + GRID_HEADER).offsetHeight;
        ...
        var pagerHeight = this.element.querySelector('.' + GRID_PAGER) ? this.element.querySelector('.' + GRID_PAGER).offsetHeight : 0;
        gridHeight = parHeight - (gBarHeight + toolBarHeight + pagerHeight) - 1;
        if (elementCreated) {
          var contentHeight = this.internalGrid && this.internalGrid.options && !isNaN(this.internalGrid.options.height) ? Number(this.internalGrid.options.height) : this.element.querySelector('.' + GRID_CONTENT + ' .' + TABLE).offsetHeight;
          var tableWidth = this.element.querySelector('.' + GRID_CONTENT + ' .' + TABLE).offsetWidth;
          var contentWidth = this.element.querySelector('.' + GRID_CONTENT + ' .' + TABLE).offsetWidth;
          var horizontalOverflow = contentWidth <= tableWidth;
          if (this.options.enableVirtualization) {
            var scrollBarHeight = this.element.querySelector('.' + 'e-scrollbar') ? this.element.querySelector('.' + 'e-scrollbar').offsetHeight : 0;
            gridHeight = horizontalOverflow ? gridHeight - scrollBarHeight : gridHeight;
          }
      ...
      cellInfo['gridHeight'] = gridHeight < this.gridSettings.rowHeight ? this.gridSettings.rowHeight.toString() : tableHeight < gridHeight ? 'auto' : gridHeight.toString();
      return JSON.stringify(cellInfo);
    };
    SfPivotView.prototype.updateGridUI = function (element, options, columnCount, rowCount) {
      ...
      if (this.options.showGroupingBar && this.groupingBarModule) {
        ...
        var emptyRowHeader = element.querySelector('.' + HEADERCONTENT).querySelector('.' + FREEZED_CELL);
      ...
      if (element.querySelector('.' + HEADERCONTENT + ' .' + TABLE) && element.querySelector('.' + HEADERCONTENT + ' .' + TABLE).style.width === '0px') {
        element.querySelector('.' + GRID_CONTENT + ' .' + TABLE).style.width = element.querySelector('.' + HEADERCONTENT + ' .' + TABLE).style.width = '';
      }
      if (element.querySelector('.' + HEADERCONTENT + ' .' + TABLE + ' th.e-firstcell.e-lastcell') && element.querySelector('.' + HEADERCONTENT + ' .' + TABLE + ' th.e-firstcell.e-lastcell').offsetWidth <= 0) {
        var headerColGroupElements = element.querySelector('.' + HEADERCONTENT + ' .' + TABLE + ' colgroup').children;
        var contentColGroupElements = element.querySelector('.' + GRID_CONTENT + ' .' + TABLE + ' colgroup').children;
      ...
      // let frozenHeader: HTMLElement = element.querySelector('.' + cls.HEADERCONTENT + ' .' + cls.TABLE);
      // let frozenContent: HTMLElement = element.querySelector('.' + cls.GRID_CONTENT + ' .' + cls.TABLE);
      // if(frozenHeader) {
      //     let frozenHeaderWidth: string = (frozenHeader.querySelector('colgroup').children[0] as HTMLElement).style.width;
      //     frozenHeader.style.width = frozenHeaderWidth;
      // }
      // if(frozenContent) {
      //     let frozenContentWidth: string = (frozenContent.querySelector('colgroup').children[0] as HTMLElement).style.width;
      //     frozenContent.style.width = frozenContentWidth;
      //     if(element.querySelector('.e-frozenscrollbar')) {
      //         (element.querySelector('.e-frozenscrollbar') as HTMLElement).style.width = frozenContentWidth;
      //     }
      // }
      if (options && this.virtualScrollModule) {
        this.virtualScrollModule.updateScrollInfo(element, columnCount, rowCount);
      }
      // let content: HTMLElement = element.querySelector('.' + cls.GRIDCONTENT_DIV + ' .' + cls.GRID_CONTENT);
      // if (content) {
      //     const isVScrollBarDisabled: boolean = !(content.scrollHeight > content.clientHeight);
      //     if (isVScrollBarDisabled){
      //         setStyleAttribute(content, {
      //             'overflow': 'hidden auto',
      //         });
      //     }
      //     if (element.querySelector('.' + cls.GRID_HEADER)) {
      //         setStyleAttribute(element.querySelector('.' + cls.GRID_HEADER), this.options.enableRtl ? {
      //             'paddingLeft': isVScrollBarDisabled ? '' : '16px'
      //         } : {
      //             'paddingRight': isVScrollBarDisabled ? '' : '16px'
      //         });
      //     }
      //     if (element.querySelector('.' + cls.HEADERCONTENT)) {
      //         setStyleAttribute(element.querySelector('.' + cls.HEADERCONTENT), this.options.enableRtl ? {
      //             'borderLeftWidth': isVScrollBarDisabled ? '' : '1px'
      //         } : {
      //             'borderRightWidth': isVScrollBarDisabled ? '' : '1px'
      //         });
      //     }
      // }
      // let scrollerDiv: HTMLElement = element.querySelector('.' + cls.MOVABLESCROLL_DIV);
      // let scrollThumb: HTMLElement = scrollerDiv && scrollerDiv.querySelector('.' + cls.MOVABLECHILD_DIV);
      // if (scrollerDiv && scrollThumb && !(Number(scrollThumb.style.width.split('px')[0]) > Number(scrollerDiv.style.width.split('px')[0]))) {
      //     (element.querySelector('.e-scrollbar') as HTMLElement).style.display = 'none';
      //     (element.querySelector('.' + cls.GRIDCONTENT_DIV + ' .' + cls.FROZENCONTENT_DIV) as HTMLElement).style.borderBottom = '0px';
      // }
    };
    SfPivotView.prototype.getScrollBarWidth = function () {
      if (!sf.base.isNullOrUndefined(this.scrollWidth)) {
    ...
    SfPivotView.prototype.destroy = function (isDestroyed) {
      this.unWireEvents();
      this.keyboardModule.destroy();
      this.commonActionModule.destroy();
      if (this.options.showGroupingBar) {
      ...
      if (this.options.showFieldList && this.pivotFieldListModule && !this.pivotFieldListModule.isDestroyed) {
        this.pivotFieldListModule.destroy(isDestroyed);
      }
      if (this.options.enableVirtualization && this.virtualScrollModule) {
  ...
  var PivotView = {
    initialize: function initialize(dataId, element, options, dotnetRef) {
      ...
      new SfPivotView(dataId, element, options, dotnetRef);
    },
    initializeFieldList: function initializeFieldList(dataId, element, options, dotnetRef) {
      sf.base.enableBlazorMode();
      new SfPivotFieldList(dataId, element, options, dotnetRef);
    },
    contentReady: function contentReady(dataId, options, refreshContent) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.options = options;
        instance.getOptions(instance.element, options);
        if (!refreshContent) {
          instance.contentReady();
        }
      }
    },
    getChartHeight: function getChartHeight(dataId, height) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.getChartHeight(height);
      }
    },
    onShowFieldList: function onShowFieldList(dataId, dialogElementId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.onShowFieldList(instance.element, document.getElementById(dialogElementId));
      }
    },
    removeFieldListIcon: function removeFieldListIcon(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.removeFieldListIcon(instance.element);
      }
    },
    updateFieldListIcons: function updateFieldListIcons(dataId, id, dragText) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.treeRendererModule.updateFieldListIcons(id, dragText);
      }
    },
    setPivotButtonDraggable: function setPivotButtonDraggable(dataId, options) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element && instance.pivotButtonModule) {
        instance.options = options;
        instance.getOptions(instance.element, options);
        instance.pivotButtonModule.createPivotButtonDrop();
        instance.pivotButtonModule.setPivotButtonDrag();
      }
    },
    isFullRowElement: function isFullRowElement(dataId, top, left) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.commonActionModule.isFullRowElement(top, left);
      }
      return JSON.stringify(false);
    },
    updateActiveNode: function updateActiveNode(dataId, treeElementId, id) {
      var isAllNode = false;
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        isAllNode = instance.commonActionModule.updateActiveNode(document.getElementById(treeElementId), id);
      }
      return JSON.stringify(isAllNode);
    },
    updateSelectedNodes: function updateSelectedNodes(dataId, node, checkState) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.updateSelectedNodes(window.sfBlazor.getElementByXpath(node), checkState);
      }
    },
    updateFieldList: function updateFieldList(dataId, containerElementID) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.updateFieldList(document.getElementById(containerElementID));
      }
    },
    renderToolbar: function renderToolbar(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.toolbarModule.renderToolbar();
      }
    },
    focusToolBar: function focusToolBar(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.toolbarModule.focusToolBar();
      }
    },
    getButtonPosition: function getButtonPosition(dataId, target, droppedClass) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return JSON.stringify(instance.pivotButtonModule.getButtonPosition(window.sfBlazor.getElementByXpath(target), droppedClass));
      }
      return null;
    },
    selectInputRange: function selectInputRange(dataId, dialogElementId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.toolbarModule.selectInputRange(document.getElementById(dialogElementId));
      }
    },
    copyMdxQuery: function copyMdxQuery(dataId, dialogElementId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.toolbarModule.copyMdxQuery(document.getElementById(dialogElementId));
      }
    },
    validateInputs: function validateInputs(dataId, filterInfo) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.commonActionModule.validateInputs(filterInfo);
      }
    },
    selectedCell: function selectedCell(dataId, colIndex, rowIndex, isHeader) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.selectedCell(colIndex, rowIndex, isHeader);
      }
      return undefined;
    },
    hyperlinkCellclick: function hyperlinkCellclick(dataId, args, XPath) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.hyperlinkCellclick(JSON.parse(args), JSON.parse(XPath));
      }
    },
    updateEditOptions: function updateEditOptions(dataId, accordId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.calculatedFieldModule.updateEditOptions(accordId);
      }
    },
    accordionClick: function accordionClick(dataId, clientX, clientY, id) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.calculatedFieldModule.accordionClick(clientX, clientY, id);
      }
      return undefined;
    },
    getAccordionValue: function getAccordionValue(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.calculatedFieldModule.getAccordionValue();
      }
      return undefined;
    },
    updateAccordionLabel: function updateAccordionLabel(dataId, target) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.calculatedFieldModule.updateAccordionLabel(target);
      }
    },
    getNodeLocation: function getNodeLocation(dataId, treeElementId, id) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.calculatedFieldModule.getNodeLocation(document.getElementById(treeElementId), id);
      }
      return undefined;
    },
    getIconInfo: function getIconInfo(dataId, top, left) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.calculatedFieldModule.getIconInfo(top, left);
      }
      return undefined;
    },
    emptyFieldName: function emptyFieldName(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.calculatedFieldModule.emptyFieldName(instance.element.id);
      }
    },
    updateCalculatedFields: function updateCalculatedFields(dataId, isEdit, top, left, title) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.calculatedFieldModule.editCalculatedFieldInfo(isEdit, top, left, title);
      }
    },
    updateCalculatedFieldExpandIcons: function updateCalculatedFieldExpandIcons(dataId, treeElementId, id) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.calculatedFieldModule.updateNodeExpandIcons(document.getElementById(treeElementId), id);
      }
      return JSON.stringify(false);
    },
    createFormulaDroppable: function createFormulaDroppable(dataId, edit, drag, remove$$1, edited, isEdit, fieldName, id) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.calculatedFieldModule.createFormulaDroppable(edit, drag, remove$$1, edited, isEdit, fieldName, id);
      }
    },
    createSheet: function createSheet(dataId, format, id, index) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.createSheet(format, id, index);
      }
    },
    updateScrollInfo: function updateScrollInfo(dataId, columnCount, rowCount) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.virtualScrollModule.updateScrollInfo(instance.element, columnCount, rowCount);
      }
    },
    getScrollInfo: function getScrollInfo(dataId, scrollPageInfo) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.getScrollInfo(scrollPageInfo);
      }
    },
    exportDocument: function exportDocument(dataId, filename, bytesBase64) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.exportDocument(filename, bytesBase64);
      }
    },
    calculateGridHeight: function calculateGridHeight(dataId, elementCreated, rowCount, columnCount) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.calculateGridHeight(elementCreated, rowCount, columnCount);
      }
      return null;
    },
    onContextMenuOpen: function onContextMenuOpen(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.onContextMenuOpen();
      }
      return null;
    },
    getSelectedCells: function getSelectedCells(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.getSelectedCells();
      }
      return null;
    },
    updateGridUI: function updateGridUI(dataId, options, columnCount, rowCount) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.updateGridUI(instance.element, options, columnCount, rowCount);
      }
    },
    updateView: function updateView(dataId, displayOption) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.updateView(instance.element, displayOption);
      }
    },
    updateGridSettings: function updateGridSettings(dataId, gridSetting, isSelectionUpdated) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.updateGridSettings(instance.element, gridSetting, isSelectionUpdated);
      }
    },
    getClientWidth: function getClientWidth(dataId, id) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.getClientWidth(instance.element, id);
      }
      return null;
    },
    getTableCellNode: function getTableCellNode(dataId, top, left) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.getTableCellNode(instance.element, top, left);
      }
      return null;
    },
    updateColorPickerUI: function updateColorPickerUI(dataId, dialogElementId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.updateColorPickerUI(document.getElementById(dialogElementId));
      }
    },
    drillthroughIntialize: function drillthroughIntialize(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.drillthroughIntialize(instance.element);
      }
    },
    getElementInfo: function getElementInfo(dataId) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        var domInfo = instance.element.getBoundingClientRect();
        var cellInfo = {
          width: domInfo.width,
          height: domInfo.height,
          clientWidth: instance.element.clientWidth,
          clientHeight: instance.element.clientHeight
        };
        return JSON.stringify(cellInfo);
      }
      return null;
    },
    getAggregateIcoPosition: function getAggregateIcoPosition(dataId, id) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        return instance.commonActionModule.getAggregateIcoPosition(id);
      }
      return null;
    },
    destroy: function destroy(dataId, isDestroyed) {
      var instance = window.sfBlazor.getCompInstance(dataId);
      if (instance && instance.element) {
        instance.destroy(isDestroyed);
      }
    }
  };
  return PivotView;
...
}]);(async()=>{await import(`${document.baseURI}_content/Syncfusion.Blazor/scripts/syncfusion-blazor-base.min.js?v=19.3.1`).then(()=>{sfBlazor.loadDependencies('sfpivotview');})})();